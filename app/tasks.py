...
async def fetch_repo(
    repo_obj: Repo, session: AsyncSession, bot: Bot, github_obj: Github
) -> Repository | None:
    try:
...
async def poll_github(
    ...
    for repo_obj in await session.scalars(
        select(Repo).options(selectinload(Repo.chat_repos).joinedload(ChatRepo.chat))
    ):
        # TODO: Filter blocked repos from SQL query
        if repo_obj.blocked or not (
            repo := await fetch_repo(repo_obj, session, bot, github_obj)
        ):
            continue
        ...
        release_or_tag, prerelease = await store_latest_release(
            ...
            repo,
            repo_obj,
            process_pre_release=settings.service.PROCESS_PRE_RELEASES,
        )
        if isinstance(release_or_tag, GitRelease):
            ...
            for chat_repo in repo_obj.chat_repos:
                message, parse_mode, entities = format_release_message(
                    chat_repo.chat.release_note_format, repo, release, github_obj
                )
                await _notify_user(
                    message,
                    chat_repo.chat.id,
    ...
    for chat in await session.scalars(stmt):
        ...
        for chat_repo in chat.chat_repos:
            try:
                ...
                if e.status == 451:
                    message = f"GitHub repo {chat_repo.repo.full_name} has been blocked"
                    logger.info(message)
                else:
                    raise
                continue
...
async def clear_db(session: FromDishka[AsyncSession]):
    for repo_obj in await session.scalars(
